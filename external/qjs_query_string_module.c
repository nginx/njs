...
static const JSCFunctionListEntry qjs_querystring_export[] = {
    ...
    JS_CFUNC_DEF("stringify", 3, qjs_query_string_stringify),
...
{
    int64_t    max_keys;
    JSValue    options, val, decode, native;
    ...
    if (JS_IsObject(options)) {
        val = JS_GetPropertyStr(cx, options, "maxKeys");
        if (!JS_IsException(val) && !JS_IsUndefined(val)) {
            if (JS_ToInt64(cx, &max_keys, val) < 0) {
                JS_FreeValue(cx, val);
                goto fail;
            }
        ...
        val = JS_GetPropertyStr(cx, options, "decodeURIComponent");
        if (!JS_IsException(val) && !JS_IsUndefined(val)) {
            if (!JS_IsFunction(cx, val)) {
    ...
    if (JS_IsNullOrUndefined(decode)) {
        val = JS_GetPropertyStr(cx, this_val, "unescape");
        if (JS_IsException(val)) {
            goto fail;
        }

        if (JS_IsUndefined(val) || !JS_IsFunction(cx, val)) {
            JS_ThrowTypeError(cx, "QueryString.unescape is not a function");
            JS_FreeValue(cx, val);
            goto fail;
        }

        native = JS_GetPropertyStr(cx, val, "native");
    ...
    val = qjs_query_string_parser(cx, str.start, str.start + str.length,
                                  sep.start ? &sep : NULL,
    ...
    njs_str_t *sep, njs_str_t *eq, JSValue decode, unsigned max_keys)
{
    ...
    while (key < end) {
        ...
        ret = qjs_query_string_append(cx, obj, key, size, val, part - val,
                                      decode);
        if (JS_IsException(ret)) {
            JS_FreeValue(cx, obj);
            return ret;
    ...
    JSValue value, njs_str_t *eq, JSValue encoder)
{
    ...
    rc = qjs_query_string_encoder_call(cx, chain, encoder, key);
    if (rc != 0) {
        return rc;
    }
...
qjs_query_string_stringify(JSContext *cx, JSValueConst this_val, int argc,
    JSValueConst *argv)
{
    JSValue    options, val, ret, encode;
    njs_str_t  sep, eq;
    ...
    options = argv[3];
    if (JS_IsObject(options)) {
        val = JS_GetPropertyStr(cx, options, "encodeURIComponent");
        if (!JS_IsException(val) && !JS_IsUndefined(val)) {
            if (!JS_IsFunction(cx, val)) {
    ...
    if (JS_IsNullOrUndefined(encode)) {
        val = JS_GetPropertyStr(cx, this_val, "escape");
        if (JS_IsException(val)) {
            goto fail;
        }
...
{
    int      rc;
    JSValue  proto, unescape;

    proto = JS_NewObject(ctx);
    JS_SetPropertyFunctionList(ctx, proto, qjs_querystring_export,
                               njs_nitems(qjs_querystring_export));

    unescape = JS_GetPropertyStr(ctx, proto, "unescape");
    if (JS_IsException(unescape)) {
        return -1;
    }

    /* Marking the default unescape function for the fast path. */

    if (JS_SetPropertyStr(ctx, unescape, "native", JS_NewBool(ctx, 1)) < 0) {
