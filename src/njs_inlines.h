...
njs_inline njs_int_t
njs_flathsh_find1(const njs_flathsh_t *fh, njs_flathsh_query_t *fhq)
{
    ...
    while (elt_num != 0) {
        ...
        njs_value_t *v =(njs_value_t *)e->value;
        if (njs_fast_path(v->atom_id == fhq->key_hash)) {
            fhq->value = v;
            return NJS_OK;
        }
        elt_num = e->next_elt;
...
njs_inline njs_int_t
njs_flathsh_insert2(njs_flathsh_t *fh, njs_flathsh_query_t *fhq)
{
    njs_int_t            cell_num;
    ...
    cell_num = fhq->key_hash & h->hash_mask;
    uint32_t *p = &(njs_hash_cells_end(h)[-cell_num - 1]);
    elt->next_elt = *p;
    *p = h->elts_count;
    ...
    njs_object_t *object, njs_value_t *key)
{
    ...
    do {
        ...
        ret = njs_flathsh_find1(&proto->hash, &pq->lhq);
        ...
        if (ret == NJS_OK) {
            ...
            ret = njs_flathsh_find1(&proto->shared_hash, &pq->lhq);
            if (ret == NJS_OK) {
