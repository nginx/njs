    ...
    void *tag)
{
    ...
    shm_zone = ngx_shared_memory_add(cf, &name,
                                     size + sizeof(ngx_slab_pool_t)
                                          + sizeof(ngx_js_shared_array_sh_t),
                                     tag);
    if (shm_zone == NULL) {
        return NGX_CONF_ERROR;
    }
...
ngx_js_shared_array_init_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    ...
    if (shm_zone->shm.exists) {
        array->sh = (ngx_js_shared_array_sh_t *) shpool->data;
        array->data = (u_char *) array->sh + sizeof(ngx_js_shared_array_sh_t);
        return NGX_OK;
    }

    array->sh = (ngx_js_shared_array_sh_t *)
                    ((u_char *) shpool + sizeof(ngx_slab_pool_t));
    array->data = (u_char *) array->sh + sizeof(ngx_js_shared_array_sh_t);
    shpool->data = array->sh;
