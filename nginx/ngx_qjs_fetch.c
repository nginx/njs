...
static JSValue
ngx_qjs_fetch_promissified_result(JSContext *cx, JSValue result, njs_int_t rc)
{
    njs_int_t  ret;
    JSValue    callback, promise, arguments[2];

    promise = JS_NewPromiseCapability(cx, arguments);
    if (JS_IsException(promise)) {
        return JS_EXCEPTION;
    }

    callback = arguments[rc != NJS_OK];

    ret = ngx_qjs_call(cx, callback, &result, 1);
    ...
    if (ret == NJS_ERROR) {
        ...
        return JS_ThrowInternalError(cx, "internal error");
    }
    ...
    int argc, JSValueConst *argv, int magic)
{
    ...
    return ngx_qjs_fetch_promissified_result(cx, result, NJS_OK);
}
    ...
    int argc, JSValueConst *argv, int magic)
{
    ...
    return ngx_qjs_fetch_promissified_result(cx, result, ret);
}
...
static const JSClassDef  ngx_qjs_fetch_class_ptr[3] = {
