...
http {
    ...
    server {
        ...
        location /https_via_proxy {
            js_fetch_proxy http://forward.proxy.net:%%PORT_8082%%;
    ...
    server {
        ...
        location = /test {
            return 200 "ORIGIN:HTTPS:response:$connection_requests";
        }
    }
}
...
like(http_get('/https_via_proxy_keepalive?domain=example.com'),
	qr/ORIGIN:HTTPS:response:1$/, 'https keepalive through proxy 1');
like(http_get('/https_via_proxy?domain=example.com'),
	qr/ORIGIN:HTTPS:response/, 'https through proxy');
like(http_get('/https_via_proxy_keepalive?domain=example.com'),
	qr/ORIGIN:HTTPS:response:2$/, 'https keepalive through proxy 2');
like(http_get('/https_no_proxy?domain=example.com'),
	qr/ORIGIN:HTTPS:response/, 'https without proxy');
			...
			if (exists $s->{pending}->{$fd}) {
				...
				if ($p->{buf} =~ /(\x0d\x0a)\1/s) {
					...
					if (uc($method) eq 'CONNECT'
						&& $proxy_auth =~ /^Basic\s+dGVzdHVzZXI6dGVzdHBhc3M=$/i)
					{
