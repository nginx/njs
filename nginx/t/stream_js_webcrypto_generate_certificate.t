...
my $t = Test::Nginx->new()->has(qw/http rewrite stream stream_return http_ssl socket_ssl/)
...
stream {
    ...
    # SSL stream using generated certificates
    server {
    ...
    # SSL stream using certificate data variables
    server {
...
http {
    ...
    # HTTP endpoints for test result retrieval (testing WebCrypto in context of stream module)
    server {
...
$t->write_file('test.js', <<EOF);
    ...
    async function get_cert_str() {
        const cert = fs.readFileSync(ngx.conf_prefix+"rsa_test_cert.pem");
        return cert;
    }

    async function get_key_str() {
        const key = fs.readFileSync(ngx.conf_prefix+"rsa_test_key.pem");
        return key;
    }
...
# Test 10: Use generated certificate from file (stream SSL)
like(stream_ssl_get('default.example.com', port(8082)),
    qr!BACKEND OK!, 'stream SSL access with file certificates');

# Test 11: Use generated certificate from string (stream SSL)  
like(stream_ssl_get('default.example.com', port(8083)),
    qr!BACKEND OK!, 'stream SSL access with data certificates');
...
sub stream_get {
	...
	# Send test data to trigger stream processing
	print $s "TEST\n";
